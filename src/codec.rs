...
pub fn codec_type(codec: &MediaCodec) -> CodecType {
  match codec {
    MediaCodec::Av1 | MediaCodec::Vp8 | MediaCodec::Vp9 | MediaCodec::H264 | MediaCodec::H265 => {
      CodecType::Video
    }
...
pub fn is_codec_supported(codec: &str) -> bool {
  matches!(
    ...
    "av1" | "vp8" | "vp9" | "h264" | "h265" | "opus" | "vorbis" | "pcm"
  )
}
...
mod tests {
  ...
  fn test_codec_type() {
    ...
    assert_eq!(
      codec_type(&MediaCodec::Unknown("test".to_string())),
      CodecType::Unknown
    );
  }
